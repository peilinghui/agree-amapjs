...
import { recursiveMerge, REG_SCRIPTS, REG_SASS_SASS, REG_SASS_SCSS, REG_LESS, REG_STYLUS, REG_STYLE, REG_MEDIA, REG_FONT, REG_IMAGE } from '@agreejs/helper'
import { getSassLoaderOption } from '@agreejs/runner-utils'
...
import { PostcssOption, IPostcssOption, ICopyOptions } from '@agreejs/taro/types/compile'
...
export const getModule = (appPath: string, {
  ...
  rule.script = {
    test: REG_SCRIPTS,
    exclude: [filename => /@agreejs\/components/.test(filename) || (/node_modules/.test(filename) && !(/taro/.test(filename)))],
    use: {
