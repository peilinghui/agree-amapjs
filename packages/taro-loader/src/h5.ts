...
import { AppConfig } from '@agreejs/taro'
...
`
  ...
  const webComponents = `
import { defineCustomElements, applyPolyfills } from '@agreejs/components/loader'
import '@agreejs/components/dist/taro-components/taro-components.css'
${extraImportForWeb || ''}
...
`
  ...
  const code = `import { createRouter, initPxTransform } from '@agreejs/taro'
import component from ${stringify(join(dirname(this.resourcePath), options.filename))}
import { ${creator}, window } from '@agreejs/runtime'
${importFrameworkStatement}
${components}
