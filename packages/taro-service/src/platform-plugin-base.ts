import type { RecursiveTemplate, UnRecursiveTemplate } from '@agreejs/shared'
...
`))
  ...
  /**
   * 返回当前项目内的 @agreejs/mini-runner 包
   */
  protected async getRunner () {
    const { appPath } = this.ctx.paths
    const { npm } = this.helper
    const runner = await npm.getNpmPkg('@agreejs/mini-runner', appPath)
  ...
  /**
   * 调用 mini-runner 开始编译
   * @param extraOptions 需要额外传入 @agreejs/mini-runner 的配置项
   */
