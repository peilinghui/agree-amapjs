...
module.exports = (_, options = {}) => {
  ...
  const nativeApis = require('@agreejs/taro-rn/apiList.js')
  const nativeLibs = require('@agreejs/taro-rn/libList.js')
  ...
  plugins.push(
    ...
    [require('babel-plugin-transform-imports-api').default, {
      packagesApis: new Map([
        ['@agreejs/taro', new Set(nativeInterfaces)],
        ['@agreejs/taro-rn', new Set(nativeInterfaces)]
      ]),
      usePackgesImport: true, // Whether to use packagesImport
      packagesImport: {
        '^@agreejs/components(-rn)?$': {
          transform: '@agreejs/components-rn/dist/components/${member}'
        },
        '^@agreejs/taro(-rn)?$': {
          transform: (importName) => {
            if (nativeLibs.includes(importName)) {
              return `@agreejs/taro-rn/dist/lib/${importName}`
            } else {
              return '@agreejs/taro-rn/dist/api'
            }
          },
