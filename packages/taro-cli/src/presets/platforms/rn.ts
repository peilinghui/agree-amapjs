...
import { chalk } from '@agreejs/helper'

function checkReactNativeDependencies (packageInfo): boolean {
  const packageNames = ['react', 'react-native', '@agreejs/taro-rn', '@agreejs/rn-runner']
...
function makeSureReactNativeInstalled (workspaceRoot: string): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    ...
    if (checkReactNativeDependencies(packageInfo)) {
      ...
      let packages = `react@^17.0.0 react-native@^0.64.0 @agreejs/taro-rn${devTag} @agreejs/rn-runner${devTag}`
  ...
  ctx.registerPlatform({
    ...
    async fn ({ config }) {
      ...
      makeSureReactNativeInstalled(appPath).then(async () => {
        // build with metro
        const rnRunner = await npm.getNpmPkg('@agreejs/rn-runner', appPath)
