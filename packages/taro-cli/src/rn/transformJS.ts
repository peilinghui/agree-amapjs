...
import wxTransformer from '@agreejs/transformer-wx'
import {
  REG_STYLE,
  REG_TYPESCRIPT,
  REG_SCRIPTS,
...
let taroImportDefaultName // import default from @agreejs/taro
...
const PACKAGES = {
  '@agreejs/taro': '@agreejs/taro',
  '@agreejs/taro-rn': '@agreejs/taro-rn',
  '@agreejs/taro-router-rn': '@agreejs/taro-router-rn',
  '@agreejs/redux': '@agreejs/redux',
  '@agreejs/components': '@agreejs/components',
  '@agreejs/components-rn': '@agreejs/components-rn',
  react: 'react',
  'react-native': 'react-native',
  'react-redux-rn': '@agreejs/taro-redux-rn',
  '@agreejs/mobx': '@agreejs/mobx',
  '@agreejs/mobx-rn': '@agreejs/mobx-rn'
}
...
export function parseJSCode ({ code, filePath, isEntryFile, projectConfig }) {
  ...
  traverse(ast, {
    ...
    ImportDeclaration (astPath) {
      ...
      if (value === PACKAGES['@agreejs/taro']) {
        ...
        source.value = PACKAGES['@agreejs/taro-rn']

        if (taroApisSpecifiers.length) {
          astPath.insertBefore(t.importDeclaration(taroApisSpecifiers, t.stringLiteral(PACKAGES['@agreejs/taro-rn'])))
        }
        if (!specifiers.length) {
          astPath.remove()
        }
      } else if (value === PACKAGES['@agreejs/redux']) {
      ...
      } else if (value === PACKAGES['@agreejs/mobx']) {
        const specifier = specifiers.find(item => {
          return t.isImportSpecifier(item) && item.imported.name === providerComponentName
        })
        if (specifier) {
        ...
        source.value = PACKAGES['@agreejs/mobx-rn']
      } else if (value === PACKAGES['@agreejs/components']) {
        source.value = PACKAGES['@agreejs/components-rn']
      }
    },
    ...
    Program: {
      exit (astPath) {
        ...
        // import Taro from @agreejs/taro-rn
        if (taroImportDefaultName) {
          const importTaro = template(
            `import ${taroImportDefaultName} from '${PACKAGES['@agreejs/taro-rn']}'`,
        ...
        if (isEntryFile) {
          ...
          // import @agreejs/taro-router-rn
          const importTaroRouter = template(
            `import TaroRouter from '${PACKAGES['@agreejs/taro-router-rn']}'`,
            babylonConfig as any
          )()
          ...
          const importTCRNProvider = template(
            `import { Provider as ${taroComponentsRNProviderName} } from '${PACKAGES['@agreejs/components-rn']}'`,
            babylonConfig
          )()
  ...
  const plugins = [
    ...
    [require('babel-plugin-danger-remove-unused-import'), { ignore: ['@agreejs/taro', 'react', 'react-native', 'nervjs'] }],
