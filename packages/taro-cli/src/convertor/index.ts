...
import { AppConfig, TabBar } from '@agreejs/taro'
import * as prettier from 'prettier'
import traverse, { NodePath } from 'babel-traverse'
import * as t from 'babel-types'
import * as taroize from '@agreejs/taroize'
    ...
    traverse(ast, {
      Program: {
        ...
        exit (astPath) {
          ...
          const hasTaroImport = bodyNode.some(p => p.isImportDeclaration() && p.node.source.value === '@agreejs/taro')
          if (needInsertImportTaro && !hasTaroImport) {
            (astPath.node as t.Program).body.unshift(
              t.importDeclaration([t.importDefaultSpecifier(t.identifier('Taro'))], t.stringLiteral('@agreejs/taro'))
            )
          }
          astPath.traverse({
...
${code}
  ...
  generateConfigFiles () {
    ...
    creator.fs.commit(() => {
      ...
      pkgObj.dependencies['@agreejs/with-weapp'] = `^${version}`
